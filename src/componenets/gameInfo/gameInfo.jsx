...
const GameInfo=(props)=>{
    ...
    let addOrRemoveCart=()=>{
        (product.tba) ? props.removeInCart(product.id,'game') : props.addToCart(product,product.id,'game')
    }
        ...
        <div className={style.product}>
            ...
            <div className={style.product__inner}>
                <div className={style.product__video}>
                    <img className={style.product__img} src={product.background_image}></img>
                    <i onClick={playVideo} class="fa fa-play-circle"></i>
                </div>
                <div className={style.product__box}>
                    <div className={style.product__box__title__made}>Made By : Rockstar Games</div>
                    <div className={style.product__box__title__price}>Price : ${product.metacritic}</div>
                    ...
                    <div>
                        <Link to={'/videogames'}><button className={style.product__box__btn__back}>Back To Products</button></Link>
                        <button onClick={addOrRemoveCart}  className={style.product__box__btn__add}>{addOrRemoveText}</button>
                    </div>
                </div>
            </div>
            <Popup editeModeP={editeMode} playVideoF={playVideo} img={product.background_image} videoUrl={mp4Url}/>
        </div>
    )
}
...
const Popup=(props)=>{
    ...
    const popupInner= <div><ReactPlayer playing={props.editeModeP} controls url={props.videoUrl} /></div>
    const popup=<div>
                    <animated.div style={fade} className={style.popup}><button onClick={props.playVideoF} >&times;</button>{popupInner}</animated.div>
                    <animated.div style={fadeBtn} onClick={props.playVideoF} className={style.overlay}></animated.div>
                </div> 
    return(
        <div >
            {popup}
